...
export default function ProfileList() {
  ...
  return (
    <div className="space-y-4">
      {menuItems.map((item) => (
        <div key={item.id} className="flex items-center p-3 border rounded-lg shadow-sm border-black bg-white">
          {/* Icon Section */}
          <div className="w-10 h-10 flex items-center justify-center border border-black rounded-full bg-white">
            {item.icon}
          </div>
          {/* Text Section */}
